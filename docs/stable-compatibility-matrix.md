...
| `GET` | `/web/osu-getreplay.php` | Missing | `required` | Replay download PHP route: target captures confirm `primary_target_client_route`, query keys `c`/`h`/`m`/`u`, and auth fields `h`/`u`; no Athena route exists yet. Success body remains blocked by `target_body_validation_requires_local_raw_blob_artifact` with `download_body_strategy=blocked`; auth failure, hidden, and storage-missing branches are fixture-backed; missing replay is unresolved because `bancho.py`/`deck` 404 conflicts with `lets` empty 200; malformed branches remain `unconfirmed`. Guide Replay Download section; fixtures under `tests/fixtures/stable_compatibility/replay_download/`. |
...
| Replay download PHP route | Missing | `required`; primary route/auth/request fields are target-confirmed, but success body remains blocked by `target_body_validation_requires_local_raw_blob_artifact` | Stable HTTP Endpoint Coverage row; guide Replay Download section; Reference Route Inventory `/web/osu-getreplay.php` row; fixtures under `tests/fixtures/stable_compatibility/replay_download/` | Missing implementation: no Athena route. Missing evidence: local target-body validation raw artifact is not committed, so `download_body_strategy=blocked`; missing replay has conflicting 404 vs empty 200 reference evidence; malformed request branches remain `unconfirmed`. Alias policy: `/web/replays/<id>` is `candidate_only_reference_backed`, not current target-client required. |
...
| `GET` | `/web/osu-getreplay.php` | replays | Grouped row: Replay download PHP route. Classification: `required`; evidence: guide Replay Download section and replay-download-contract fixtures. Blocker: `target_body_validation_requires_local_raw_blob_artifact`; body decision: `download_body_strategy=blocked`. |
