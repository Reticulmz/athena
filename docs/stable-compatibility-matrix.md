...
| `BEATMAP_INFO` | `c2s/beatmap_info.py::BeatmapInfoRequest` | Defines the fixture-backed current 4-Grade request payload. C2S 68 parser and handler remain missing, and Target Stable Client capture is unconfirmed; `tests/unit/transports/bancho/protocol/test_beatmap_info_fixtures.py`; verification=unit,fixture; fixture blocker=none. |
...
| `BEATMAP_INFO_REPLY` | `s2c/beatmap_info.py::BeatmapInfo`, `s2c/beatmap_info.py::BeatmapInfoReply` | Defines the fixture-backed current 4-Grade reply payload. S2C 69 builder and runtime remain missing, and Target Stable Client capture is unconfirmed; `tests/unit/transports/bancho/protocol/test_beatmap_info_fixtures.py`; verification=unit,fixture; fixture blocker=none. |
...
| Modern getscores | Implemented | `required` | `src/osu_server/composition/application.py`; `src/osu_server/transports/stable/web_legacy/getscores.py`; `src/osu_server/transports/stable/web_legacy/mappers/getscores.py`; `tests/fixtures/stable_compatibility/getscores/response_shapes.json`; `tests/fixtures/stable_compatibility/getscores/branch_cases.json`; `tests/fixtures/stable_compatibility/getscores/beatmap_status_crosswalk.json`; guide `/web/osu-osz2-getscores.php` implementation-completion evidence note | Athena-owned completion evidence is validated: five response shapes, branch catalog, status crosswalk, focused runtime tests, and stable verifier. Missing Target Stable Client traffic confirmation is separate and remains with Issue #27 / #28. |
...
| Replay download PHP route | Implemented | `required`; primary route/auth/request fields are target-confirmed, and success body uses `download_body_strategy=direct_blob_bytes` after local metadata-only validation | Stable HTTP Endpoint Coverage row; guide Replay Download section; Reference Route Inventory `/web/osu-getreplay.php` row; fixtures under `tests/fixtures/stable_compatibility/replay_download/` | Remaining gaps: missing replay has conflicting 404 vs empty 200 reference evidence; malformed request branches remain `unconfirmed`. Alias policy: `/web/replays/<id>` is `candidate_only_reference_backed`, not current target-client required. |
