...
def test_run_pytest_uses_paths_and_propagates_exit_code() -> None:
    ...
    runner = ProcessRunner(executor=executor)
    unit_test_path = "apps/athena_server/tests/unit"
    integration_test_path = "apps/athena_server/tests/integration"
    expected_argv = ("pytest", unit_test_path, integration_test_path)

    result = runner.run_pytest(
        paths=(unit_test_path, integration_test_path),
