...
_GETSCORES_COMPLETION_MANIFEST_REFERENCES = frozenset(
    (
        ...
        "apps/athena_server/tests/fixtures/stable_compatibility/getscores/branch_cases.json",
        "apps/athena_server/tests/fixtures/stable_compatibility/getscores/beatmap_status_crosswalk.json",
    )
)
_CURRENT_GUIDE_FIXTURE_REFERENCES = frozenset(
    (
        "apps/athena_server/tests/fixtures/stable_compatibility/getscores/response_shapes.json",
        "apps/athena_server/tests/fixtures/web_legacy/getscores/completion/",
        "apps/athena_server/tests/fixtures/golden/",
    )
)
_RETIRED_GUIDE_FIXTURE_REFERENCES = frozenset(
    (
        "tests/fixtures/stable_compatibility/getscores/response_shapes.json",
        "tests/fixtures/web_legacy/getscores/completion/",
...
def test_stable_compatibility_guide_references_relocated_test_fixtures() -> None:
    """Stable compatibility guideが移設済みfixtureを現在のevidenceとして示すことを検証する.

    Getscores response shape、completion body、score-submit golden fixtureはserver owner配下を
    参照し、retired root `tests` pathをcurrent evidenceとして案内しないことを確認する.

    Returns:
        None: guideのcurrent fixture referenceを検証して完了する.

    Raises:
        AssertionError: guideが移設済みfixtureを参照しないか、retired root pathを参照する場合.
    """
    guide_contents = _STABLE_COMPATIBILITY_GUIDE_PATH.read_text(encoding="utf-8")

    assert all(reference in guide_contents for reference in _CURRENT_GUIDE_FIXTURE_REFERENCES)
    assert all(
        f"`{reference}`" not in guide_contents for reference in _RETIRED_GUIDE_FIXTURE_REFERENCES
    )
