...
_OFFICIAL_GETSCORES_SOURCE_PREFIX = (
...
def test_evidence_source_is_typed_and_rejects_raw_query_like_reference(
    ...
    assert source == (
        "official_fixture:"
        "apps/athena_server/tests/fixtures/web_legacy/getscores/ranked_response.txt"
    )
...
def test_status_crosswalk_markdown_source_resolves_to_canonical_heading_anchor() -> None:
    ...
    source_path = _FIXTURE_ROOT.parents[3] / source_prefix / relative_path
...
def test_status_crosswalk_validator_rejects_official_fixture_wire_status_mismatch(
    ...
    source_path = (
        source_root / "apps" / "athena_server" / "tests" / "fixtures" / "web_legacy" / "getscores"
    )
