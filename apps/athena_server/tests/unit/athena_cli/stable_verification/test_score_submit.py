...
FIXTURE_DIR = (
    Path(__file__).resolve().parents[3] / "fixtures" / "stable_compatibility" / "score_submit"
)
...
def test_verify_response_body_maps_failed_response_to_secret_free_fail_result() -> None:
    ...
    result = ScoreSubmitVerifier(fixture_dir=FIXTURE_DIR).verify_response_body(
        body_with_secret_hint,
        reference="apps/athena_server/tests/fixtures/stable_compatibility/score_submit/failed_response.txt",
    )
