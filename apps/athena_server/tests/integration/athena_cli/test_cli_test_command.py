...
runner = CliRunner()
SERVER_TEST_ROOT_PATH = "apps/athena_server/tests"
...
def test_test_command_runs_setup_then_default_pytest_path(
    ...
    """Test commandがdatabase setup後に既定のserver test pathでpytestを実行することを検証する.
    ...
    assert process_runner.calls[0][0] == ("pytest", f"{SERVER_TEST_ROOT_PATH}/")
...
def test_test_command_passes_multiple_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    unit_test_path = f"{SERVER_TEST_ROOT_PATH}/unit"
    integration_test_path = f"{SERVER_TEST_ROOT_PATH}/integration"
    ...
    assert result.exit_code == 0
    assert process_runner.calls[0][0] == (
        "pytest",
        unit_test_path,
        integration_test_path,
    )
...
def test_test_command_propagates_pytest_exit_code(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "Command failed with exit code 5: pytest apps/athena_server/tests/" in result.output
