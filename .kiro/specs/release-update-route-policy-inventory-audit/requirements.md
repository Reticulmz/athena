...
- **In scope**: `/web/check-updates.php`、`/release/update*`、`/release/patches.php`、root `/update*` / `/patches.php` aliases、release file、filter、localization route の互換分類、response shape、evidence source、運用依存、fixture 要否の整理。
...
### Requirement 3: Release file, filter, and localization operational dependency policy
...
5. Where a release file, filter, or localization route is deferred behind an operational decision, the Athena stable compatibility audit shall mark fixture requirement as `deferred`.
