...
2. When SQL is recorded, the system shall redact SQL literal values, normalize whitespace, and group duplicate templates without using SQL parameter values.
3. When the same normalized SQL appears at or above the duplicate threshold, the system shall include it in the bounded duplicate query summary.
...
2. When a query budget failure occurs, the system shall report the scope name, actual count, allowed count, duplicate template counts, truncation state, and short redacted SQL template prefixes.
...
2. When query diagnostics are wired, the system shall keep SQLAlchemy event instrumentation in infrastructure and expose request/job scope ownership through shared application-facing primitives.
