...
def _make_help_ctx(registry: CommandRegistry) -> CommandContext:
    """Construct a CommandContext for help command using the registry's visible commands."""
    return CommandContext(
        ...
        command_name="help",
        args=(),
        available_commands=registry.visible_commands(),
    )
...
class TestBuiltinRegistryStructure:
    ...
    def test_exactly_two_visible_commands(self) -> None:
        """Builtin catalog registers exactly two player-visible commands."""
        registry = create_builtin_registry()
    ...
    def test_both_commands_are_visible(self) -> None:
        """Both roll and help are player-visible."""
        registry = create_builtin_registry()
        roll_def = registry.resolve("roll")
        help_def = registry.resolve("help")
        assert roll_def is not None
        assert help_def is not None
        assert roll_def.metadata.visible is True
        assert help_def.metadata.visible is True
