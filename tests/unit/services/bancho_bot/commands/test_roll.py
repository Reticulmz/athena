...
def roll_def() -> CommandDefinition:
    """Return the registered roll CommandDefinition."""
...
class TestRollRegisteredAsCommandDefinition:
    """Verify roll is registered via the @registry.command() decorator contract."""

    def test_roll_is_command_definition(self, roll_def: CommandDefinition) -> None:
        """The @registry.command() decorator produces a valid CommandDefinition for roll."""
    ...
    async def test_invocation_through_decorated_handler(self, roll_def: CommandDefinition) -> None:
        """Invoking roll through the decorated handler works correctly."""
