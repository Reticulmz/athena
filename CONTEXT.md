...
### Stable Grade
Stable client の `BeatmapInfo` などで使う wire 固有の grade classification。Canonical Score Grade または no-score state を compatibility boundary で `XH` から `N` の stable value に変換し、raw grade byte を domain meaning として扱わない。
...
Stable wire 固有の enum (`StableStatus`, `StableMode`, `StablePresenceFilter`, `StableGrade`) は `domain/compatibility/stable/` にファイル分割で配置する (`status.py`, `mode.py`, `presence_filter.py`, `grade.py`)。命名は `Stable` プレフィックス。
...
テストファイルは責務別に分割:
- `test_stable_enums.py`: StableStatus/Mode/PresenceFilter/Grade の値テスト
- `test_presence_fixtures.py`: UserPresence golden bytes
- `test_stats_fixtures.py`: UserStats golden bytes
- `test_beatmap_info_fixtures.py`: BeatmapInfoRequest/BeatmapInfo/BeatmapInfoReply golden bytes
