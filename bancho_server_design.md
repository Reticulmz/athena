...
- **Redis を揮発的ステートの中央ストア**として活用し、トランスポートの水平スケールとプロトコル横断的なステート整合性を実現
...
7. 揮発的ステート(セッション、プレゼンス、チャンネル状態)を Redis に集約し、プロセス再起動でセッションが消失しない構造とする
...
│ ├─ cache (キャッシュ用途の Redis ラッパー)            │
│ ├─ state (揮発的ステートの Redis ストア)             │
│ │   ├─ SessionStore                                │
│ │   ├─ PresenceStore                               │
│ │   ├─ ChannelStateStore                           │
│ │   ├─ MatchStateStore                             │
...
実行プロセスは以下の2つに分かれる。Services 以下のレイヤーは両プロセスで共有される。揮発的ステート(セッション、プレゼンス等)は Redis に集約され、永続データは PostgreSQL に格納される。
...
Redis を **揮発的ステートの中央ストア** として位置づけることで、app プロセスを水平スケールしてもセッション情報が分散せず、プロセス再起動でも接続中ユーザーのセッションが消失しない構造になる。これは bancho.py の「メモリ上のグローバルステート」設計に対する本質的な改善点である。詳細はセクション 8.5 を参照。
...
| キャッシュ / ブローカー | Redis | セッション、レート制限、キャッシュ、Pub/Sub、Streams を1つの基盤に集約 |
| イベントバス(fire-and-forget) | 自前 EventBus 抽象 + 実装(in-memory / Redis Pub/Sub) | チャット配信・プレゼンス更新等のロス許容な配信 |
| ジョブキュー(配信保証あり) | ARQ | async first、Redis ベース、軽量、型ヒント親和性。スコア後処理・通知・メダル付与等の重い処理をワーカーに委譲 |
...
チャット配信、プレゼンス更新、spectator フレーム配信などの fire-and-forget 配信に使用する。配信保証は不要、レイテンシ重視。すでに Redis を他の用途(キャッシュ、セッション)で使うため、追加インフラは発生しない。
...
3. **Redis 基盤の共有**: 追加インフラが発生しない
4. **型ヒント親和性**: 設計書の他部分(Pydantic、dataclass、型駆動)との一貫性
5. **本家 osu! も Redis 中心の構成**: `ppy/osu-queue-score-statistics` が Redis ベースのキューシステムを採用しており、参考になる

Redis Streams を直接扱うのではなく、ARQ のような上位ライブラリに任せることで、ジョブの登録・実行・再試行・ack を抽象化された API で扱える。Streams を生で扱うのは bancho サーバーの抽象化レベルとしては低すぎる。
...
├── cache/
│   ├── __init__.py
│   ├── redis_client.py            # Redis 接続(全用途で共有する基盤クライアント)
...
│   └── redis_pubsub.py            # Redis Pub/Sub 実装(本番用)
│
...
|---|---|---|---|---|
| `state/` | 揮発的ステートの読み書き(同期的) | 1 ms 未満 | Redis のメモリ上(任意で AOF) | セッション参照、プレゼンス更新、チャンネルメンバー取得 |
...
`state/` と `messaging/` の違いは「同期的にデータを読み書きするか、非同期にイベントを送るか」である。同じ Redis 基盤を使うが、API も用途も異なる。具体的な使い分けはセクション 8.5 と 8.8 を参照。
...
|---|---|---|---|---|---|
| DI コンテナ | 依存解決 | 同期 | - | - | - |
| StateStore | 揮発的ステート管理 | 同期(read/write) | Redis メモリ | - | △(明示的に書き込めば残る) |
...
def build_container(config: AppConfig) -> Container:
    ...
    # Infrastructure: 共通(Redis 接続、DB エンジン)
...
### 8.5 ステート管理戦略(Redis 中央集約)

bancho サーバーが扱うデータは、永続化が必要なもの(ユーザー、スコア、ビートマップ等)と、揮発的だが複数プロセス・複数リクエストにまたがって共有されるもの(セッション、プレゼンス、チャンネル参加状況等)に分けられる。本設計では **後者を Redis に集約** する。
...
Redis 集約はこれらをすべて解決する。

#### 8.5.1 揮発的ステートの分類

bancho サーバーが保持する揮発的データを以下に分類する。すべて Redis に置く。
...
各ステートは `infrastructure/state/interfaces/` 配下で Protocol として抽象化する。サービス層・トランスポート層は Protocol に依存し、具体実装(Redis / in-memory)を知らない。
...
`infrastructure/state/redis/keys.py` に Redis のキー命名規約を集約する。これにより、複数のストア実装間でキー名の衝突や暗黙の依存を防ぐ。

```python
# infrastructure/state/redis/keys.py
"""Redis キー命名規約。すべてのキーは小文字、コロン区切り、英数字のみを使う。"""
...
将来 Redis Cluster へ移行する可能性を考慮し、 **複数キーを atomic に操作する場合は hash tag** を使う。例えば、ユーザー単位で複数のキーを同一トランザクションで触る場合:
...
`{user:12345}` の部分が同じスロットに配置されるため、Redis Cluster 化後も MULTI/EXEC が機能する。

#### 8.5.4 アクセス権限のルール

「Redis 上の揮発的ステートに、どのレイヤーがアクセスして良いか」を明確化する。原則として **すべてのレイヤーが state Protocol を介して Redis にアクセスして良い** が、永続データ(DB)へのアクセスはサービス層に限定する。
...
`state/` と `messaging/` は同じ Redis 基盤を使うが、責務が異なる。混同しないこと。
...
これにより「状態の正本は Redis 上の state」「変更通知は EventBus」という二段構えの整合性モデルが成立する。クライアント側は通知を取りこぼしても、明示的に state を問い合わせれば最新状態を取得できる。

#### 8.5.8 in-memory 実装の活用

`state/memory/` 以下に各 Protocol の in-memory 実装を提供する。これは:

- **テスト**: Redis なしでサービス層のテストを書ける
...
ARQ ワーカーは `osu_server.worker` モジュールから起動する。app プロセスと同じ DI コンテナを構築し、ジョブ実行時に `ctx["container"]` から依存を解決する。
...
| 永続化 | Redis メモリ(任意で AOF) | なし | あり(Redis Streams) |
| 失敗時の再試行 | 関係なし | なし | あり |
...
| | 期限切れセッション清掃 | 不要データが Redis に蓄積し続ける |
...
- **app プロセスがクラッシュしても影響最小**(DB 保存後にクラッシュしても、ジョブは Redis 上に残っており worker が拾う)
...
ワーカープロセスは複数立ち上げて水平スケール可能(同じ Redis ブローカーを共有する複数のワーカーがジョブを分担する)。
...
services:
  ...
  worker:
    build: .
    command: arq osu_server.worker.WorkerSettings
    environment:
      - DATABASE_URL=postgresql+asyncpg://...
      - REDIS_URL=redis://redis:6379/0
    depends_on:
...
app プロセスがダウンしてもワーカーは独立して動作し続け、ジョブの処理は継続される。逆にワーカーがダウンしても app プロセスのリクエスト処理に影響しない(ジョブは Redis に蓄積され、ワーカー復旧後に処理される)。
...
- **RabbitMQ / Kafka の導入**: 現状の Redis(Pub/Sub + Streams via ARQ)で必要十分。複雑な routing が要求されるか、桁違いのスループットが必要になった時点で再検討
...
- Stage 1 から Redis ステート集約を採用している(セッション、プレゼンスがプロセス共有可能)
...
各サービスが独立した DB、独立したコードベース(別リポジトリ)、独立したリリースサイクルを持つ。イベントバスは Redis Pub/Sub から RabbitMQ / Kafka 等に拡張される可能性がある。
...
- **osu-state**: Redis ステートストア。 `osu-domain` に依存
...
| ARQ | Async first な Python 向けジョブキューライブラリ。Redis ベース。Sidekiq の Python 版に相当 |
| StateStore | 揮発的ステート(セッション、プレゼンス等)を Redis に保管するための Protocol 群 |
| EventBus | 配信ロスを許容する fire-and-forget な通知機構。本設計では Redis Pub/Sub をバックエンドに使用 |
| JobQueue | 配信保証ありのジョブ実行機構。失敗時に再試行される。本設計では ARQ + Redis Streams を使用 |
...
| Sidekiq | Ruby 界のデファクトジョブキュー。本設計では参考とし、Python 版相当として ARQ を採用 |
| app プロセス | uvicorn で起動する ASGI サーバープロセス。HTTP / WebSocket リクエストを処理 |
| worker プロセス | ARQ で起動するジョブ実行プロセス。app プロセスとは独立して水平スケール可能 |
...
| EventBus 実装 | Redis Pub/Sub | Redis Streams、自前実装一択 | チャット配信などロス許容な用途には Pub/Sub の単純さで十分 |
| EventBus ライブラリ | 自前 InMemoryEventBus(40 行程度) | Pyventus, blinker, pymitter, PyDispatcher 等 | クラスレベル状態(EventLinker)が DI 設計と矛盾、結局 Redis Pub/Sub を別実装する必要があり依存追加のメリットが薄い、コアな抽象化レイヤーは自前実装する原則。ただし dataclass ベースのイベント定義など思想は参考にする |
| JobQueue 実装 | ARQ | Celery, RQ, Dramatiq, TaskIQ, Redis Streams 直叩き | async first、軽量、Redis 基盤共有、型ヒント親和性 |
| メッセージング基盤 | Redis(Pub/Sub + ARQ via Streams) | RabbitMQ, Kafka | private server 規模では Redis で必要十分、運用基盤を増やさない |
...
| 揮発的ステートの保管場所 | Redis に集約(StateStore Protocol 経由) | プロセスメモリ(bancho.py 方式)、DB 直接 | プロセス再起動でセッション消失を防ぐ、トランスポート水平スケール対応、プロトコル間ステート整合性 |
| StateStore の API スタイル | Protocol(具象 in-memory / Redis 切替可能) | グローバル関数、Redis 直接呼び出し | テスト容易性、in-memory 実装での単一プロセス開発、将来的な実装差し替え |
...
- **StateStore の Redis 実装に切替**(SessionStore, PresenceStore, PacketQueue を Redis ベースに)
...
- EventBus 基盤(InMemoryEventBus 実装、Redis Pub/Sub 実装)、broadcast 機構

**Phase 4: スコア機能(stable)+ Worker プロセス導入(4〜6週間)**
- web_legacy/ の score_submission 実装
- AES 復号、クライアントハッシュ検証
- services/scoring/ の実装、リーダーボード
- **JobQueue 基盤の構築**: ARQ アダプタ、worker.py、定期実行ジョブの土台
- **スコア後処理を Worker に分離**: PP 計算、リーダーボード更新、統計更新を非同期化
- Docker Compose による app + worker + Redis + DB の統合起動
...
- spectator フレーム配信を EventBus(Redis Pub/Sub)で実装
...
各 Phase は独立して動作確認できる単位とし、毎回 stable または lazer クライアントでの実動作テストを通過させる。Phase 1 で StateStore Protocol を導入する時点で、Redis 集約への準備が整い、Phase 4 でワーカープロセスを導入する時点で、本設計の二プロセス構成が完成する重要な節目となる。Phase 9 以降の進化(Stage 2〜5)は、規模と必要性に応じて選択的に実施する。
...
| 言語 | TypeScript |
| 揮発的ステート | Durable Objects(Redis StateStore の代替) |
...
|---|---|
| StateStore (Redis) | Durable Objects(チャンネル / マッチ / ユーザー / 観戦単位でインスタンス) |
| EventBus (Redis Pub/Sub) | Durable Object 内の WebSocket fan-out + Workers 間 Service Bindings |
| JobQueue (ARQ) | Cloudflare Queues |
| 永続 DB (PostgreSQL) | Hyperdrive 経由の外部 PostgreSQL、または D1(SQLite ベース、軽量用途のみ) |
| キャッシュ (Redis) | Workers KV(結果整合性、グローバル分散) |
| ファイルストレージ | R2 |
...
| Redis クライアント | fred(高機能)または deadpool-redis | redis-py |
| ジョブキュー | apalis | ARQ |
...
#### E.5.1 StateStore(Redis 集約)

trait による抽象化と、`fred` クレートによる Redis 実装。
...
| Redis クライアント | StackExchange.Redis | redis-py |
| ジョブキュー | Hangfire または Coravel | ARQ |
...
#### F.5.1 StateStore(Redis 集約)

interface(C# の慣習でプレフィックス `I`)と StackExchange.Redis による実装:
...
- **Hangfire**: 機能豊富、Web UI 付き、Redis / SQL バックエンド対応、ARQ の C# 版に相当
...
#### F.6.3 Redis backplane で水平スケール

複数の signalr-host インスタンスをロードバランスする場合、SignalR の Redis backplane を使う:
...
これで複数の signalr プロセスが Redis 経由でメッセージを共有でき、トランスポートの水平スケールが完成する。
